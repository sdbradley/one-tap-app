...
export const AUTHENTICATION_FORGOT_PASSWORD_FAILED = "AUTHENTICATION_FORGOT_PASSWORD_FAILED";
export const AUTHENTICATION_RESET_PASSWORD_SUCCESS = "AUTHENTICATION_RESET_PASSWORD_SUCCESS";
export const AUTHENTICATION_RESET_PASSWORD_FAILED = "AUTHENTICATION_RESET_PASSWORD_FAILED";
export const AUTHENTICATION_FETCH_DATA_SUCCESS = "AUTHENTICATION_FETCH_DATA_SUCCESS";
export const AUTHENTICATION_VERIFY_EMAIL_SUCCESS = "AUTHENTICATION_VERIFY_EMAIL_SUCCESS";
export const AUTHENTICATION_VERIFY_EMAIL_FAILED = "AUTHENTICATION_VERIFY_EMAIL_FAILED";
export const AUTHENTICATION_LOGOUT = "AUTHENTICATION_LOGOUT";
export const AUTHENTICATION_INACTIVITY_WARNING = "AUTHENTICATION_INACTIVITY_WARNING";
...
function inactivityLogoutTimeout(dispatch) {
  return setTimeout(
    () => {
      ...
      dispatch(setAuthMessage({ title: 'Login', description: 'You have been logged out due to inactivity.' }));
    },
...
export function activity(token) {
  return (dispatch, getState) => dispatch(_activity(
...
export function signup({ first_name, last_name, email, password, terms }) {
  if (!terms || terms === 'false') {
    throw new Error('You must agree to the Terms and Conditions and the Privacy Policy to continue.');
  }
  return (dispatch) => API.post('public/users', {
    user: { first_name, last_name, email, password, password_confirmation: password },
    callback_url: join(CLIENT_LOCATION, 'verify', encodeURIComponent(email), '{token}')
  })
    .then(() => dispatch(currentUserSignupSuccess()))
    .catch((response) => response.json().then(
      json => dispatch(currentUserSignupFailed(json.errors))
    ));
}

export function currentUserFetchData() {
  return (dispatch, getState) => API.get('users/current')
    .then(user => {
...
export function forgotPassword({ email }) {
  return (dispatch) => API.post('public/users/password_reset', {
    user: { email },
    callback_url: join(CLIENT_LOCATION, 'forgot-password', encodeURIComponent(email), '{token}')
  })
    .then(() => dispatch(forgotPasswordSuccess()))
    .catch((response) => response.json().then(
      json => dispatch(forgotPasswordFailed(json.errors))
    ))
    .catch((response) => dispatch(forgotPasswordFailed(new Error('An error occurred while processing your request.'))));
}
...
export function resetPassword({ email, token, password, terms }) {
  ...
  return (dispatch) => API.post('public/users/password', {
    user: { email, token, password }
  })
    .then(() => dispatch(resetPasswordSuccess()))
    .catch((response) => response.json().then(
      json => dispatch(resetPasswordFailed(json.errors))
    ))
    .catch((response) => dispatch(resetPasswordFailed(new Error('An error occurred. If you have already created your account, click the link above to return to the Login screen'))));
}

export function verifyEmail({ email, token }) {
  return (dispatch) => API.post('public/token/validate', { user: { email, token } })
    .then(() => dispatch(verifyEmailSuccess()))
    .catch((response) => dispatch(verifyEmailFailed(new Error('An unknown error occured'))));
}
