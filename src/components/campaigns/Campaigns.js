...
class Campaigns extends Component {
  ...
  renderName(campaign) {
    return (
      <Link classic hard to={`${APP_ROOT}campaigns/${campaign.id}`}>
        {campaign.name}
      </Link>
    );
  }
  renderStartDate(c) {
    if (c.start_date) {
      return <Moment format="MM/DD/YYYY">{c.start_date}</Moment>;
    }
  }
  renderEndDate(c) {
    if (c.end_date) {
      return <Moment format="MM/DD/YYYY">{c.end_date}</Moment>;
    }
  }
  renderProgress(c) {
    let num_sent = c.number_sent;
    let opportunities = c.number_of_opportunities;
    let progress = num_sent > 0 ? opportunities / num_sent : 0;
    let pct = setPrecision(progress * 100, 1);
    return <div>{`${pct}%`}</div>;
  }
}
...
export default connect((state, props) => {
  ...
  let role = user.hasRole(ROLE.STAKEHOLDER)
    ? ROLE.STAKEHOLDER
